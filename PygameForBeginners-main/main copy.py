...
def yellow_handle_movement(keys_pressed, yellow,red,stunned,infinity, domainActive):
    if not stunned:
        ...
        if domainActive:
            yellow.x = red.x -30
            yellow.y = red.y
        if keys_pressed[pygame.K_a] and yellow.x - VEL > 0:  # LEFTdd
            yellow.x -= VEL
            infinity.x -= VEL
        ...
        if keys_pressed[pygame.K_s] and yellow.y + VEL + yellow.height < HEIGHT - 15:  # DOWN
            yellow.y += VEL
            infinity.y += VEL
        if GravityActive and yellow.y + GRAVITY + yellow.height < HEIGHT - 15:
            yellow.y += GRAVITY
...
yellow_punch = pygame.USEREVENT + 21
...
def main():
    ...
    red_damage = 1
    GojoStatus = 0
    yel_punch_frame_num = 1
    yel_punch_frame = ''    


    import spritesheet
    sprite_sheet_slash = pygame.image.load('Assets/GokuPunch.png')
    sprite_sheet = spritesheet.SpriteSheet(sprite_sheet_slash)

    while run:
        SpriteWidth = 81
        SpriteHeight = 100
        frame0 = sprite_sheet.get_image(0,SpriteWidth,SpriteHeight,1,PURPLE)
        frame1 = sprite_sheet.get_image(1,SpriteWidth,SpriteHeight,1,PURPLE)
        frame2 = sprite_sheet.get_image(2,SpriteWidth,SpriteHeight,1,PURPLE)
        frame3 = sprite_sheet.get_image(3,SpriteWidth,SpriteHeight,1,PURPLE)
        frame4 = sprite_sheet.get_image(4,SpriteWidth,SpriteHeight,1,PURPLE)
        frame5 = sprite_sheet.get_image(5,SpriteWidth,SpriteHeight,1,PURPLE)
        frame6 = sprite_sheet.get_image(6,SpriteWidth,SpriteHeight,1,PURPLE)
        WIN.blit(frame0, (0,0))
        WIN.blit(frame1, (120,0))
        WIN.blit(frame2, (240,0))
        WIN.blit(frame3, (360,0))
        WIN.blit(frame4, (480,0))
        WIN.blit(frame5, (600,0))
        WIN.blit(frame6, (720,0))
        yellow_punch_frames = {
            1:frame0,
            2:frame1,
            3:frame2,
        ...
        for event in pygame.event.get():
            ...
            if event.type == yellow_punch:
                if yel_punch_frame_num >= 8:
                    yel_punch_frame_num = 1
                print(yel_punch_frame_num)
                yel_punch_frame = yellow_punch_frames[yel_punch_frame_num]
                WIN.blit(yel_punch_frame, (yellow.x,yellow.y-120))
                yel_punch_frame_num += 1
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_f:
                    ...
                    #infinityActive = True
                    pygame.time.set_timer(yellow_punch,80,loops=-1)
